...
import { the_hindu_premium_ep, api_endpoints } from "@/lib/endpoints";
...
export default function TheHinduMainFeed() {
    const {
        items,
        page,
        pages,
        ...
        <section className="min-h-screen max-w-4xl mx-auto p-4">
            ...
            {items.map((item, idx) => {
                ...
                return (
                    <Link
                        key={item.id + "#" + idx}
                        href={`/the_hindu_premium/story${item.wu?.replace("https://www.thehindu.com", "") ?? ""
                            }`}
